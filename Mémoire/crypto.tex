...
%% Generated: Sun Mar 26 13:24:23 CEST 2017
...
\begin{maplegroup}
\begin{Maple Normal}{
Powers\textit{permet de calculer a\symbol{94}k mod n en décomposant k en base 2.}}\end{Maple Normal}
...
\begin{maplegroup}
\begin{mapleinput}
\mapleinline{active}{2d}{Pollard(1715761513)}{\[{\it Pollard} \left( 1715761513 \right) \]}
\end{mapleinput}
\mapleresult
\begin{maplelatex}
...
\mapleinline{inert}{2d}{Lenstra := proc (n) local c, Q, p; if n::prime then return print("n est premier") end if; b := rand(); x1 := rand(); y1 := rand(); c := `mod`(-x1^3-b*x1+y1^2, n); if 1 < gcd(4*b^3+27*c^2, n) then return gcd(4*b^3+27*c^2, n) end if; for p from 2 to factorial(1000) do Q := double(factorial(p), x1, y1, b, c, n); if Q::integer then return Q end if; b := rand(); x1 := rand(); y1 := rand(); c := `mod`(-x1^3-b*x1+y1^2, n); if 1 < gcd(4*b^3+27*c^2, n) then return gcd(4*b^3+27*c^2, n) end if end do; print("RÃ©essayer avec une nouvelle courbe et/ou point"); Q end proc; 1}{\[\displaystyle \]}
\mapleresult
\underline{}Warning, `b` is implicitly declared local to procedure `Lenstra`\underline{}\mapleresult
\underline{}Warning, `x1` is implicitly declared local to procedure `Lenstra`\underline{}\mapleresult
\underline{}Warning, `y1` is implicitly declared local to procedure `Lenstra`\underline{}\mapleresult
\begin{maplelatex}
\mapleinline{inert}{2d}{Lenstra := proc (n) local c, Q, p, b, x1, y1; if n::prime then return print("n est premier") end if; b := rand(); x1 := rand(); y1 := rand(); c := `mod`(-x1^3-b*x1+y1^2, n); if 1 < gcd(4*b^3+27*c^2, n) then return gcd(4*b^3+27*c^2, n) end if; for p from 2 to factorial(1000) do Q := double(factorial(p), x1, y1, b, c, n); if Q::integer then return Q end if; b := rand(); x1 := rand(); y1 := rand(); c := `mod`(-x1^3-b*x1+y1^2, n); if 1 < gcd(4*b^3+27*c^2, n) then return gcd(4*b^3+27*c^2, n) end if end do; print("RÃ©essayer avec une nouvelle courbe et/ou point"); Q end proc}{\[\displaystyle {\it Lenstra}\, := \,\textbf{proc} (n) \\
...
\mapleIndent{1} c\,:=\,\mathit{mod} (-x1\hat{~}{3}-b \ast x1 + y1\hat{~}{2},\,n);\\
...
\mapleIndent{2} c\,:=\,\mathit{mod} (-x1\hat{~}{3}-b \ast x1 + y1\hat{~}{2},\,n);\\
...
\mapleIndent{2} \textbf{end\ if}; \\
\mapleIndent{1} \textbf{end\ do};;\\
\mapleIndent{1} \mathit{print} (Réessayer\ avec\ une\ nouvelle\ courbe\ et/ou\ point'');\\
\mapleIndent{1} Q\\
\textbf{end\ proc};\]}
\end{maplelatex}
\end{maplegroup}
...
\begin{maplegroup}
\begin{mapleinput}
\mapleinline{active}{2d}{double(factorial(20), 2, 3, 1, -1, 1715761513)}{\[{\it double} \left( 20!,2,3,1,-1,1715761513 \right) \]}
\end{mapleinput}
\mapleresult
\begin{maplelatex}
\mapleinline{inert}{2d}{[693588502, 858100579]}{\[\displaystyle [693588502,858100579]\]}
\end{maplelatex}
\end{maplegroup}
\begin{Maple Normal}{
\begin{Maple Normal}{
...
\begin{maplegroup}
\begin{mapleinput}
\mapleinline{active}{2d}{double2 := proc (k, x1, y1, a, b, n) local r, X, Z, A, m, lambda, nu, i, x, y, Y, x3, y3, j, l; X := convert(k, 'base', 2); Z := convert(X, array); r := 0; while 2^r <= k do r := r+1 end do; A := Matrix(1, 2*r); A[1, 1] := x1; A[1, 2] := y1; for j to r-1 do if gcd(2*b*A[1, 2*j], n) = 1 then A[1, 2*j+1] := `mod`((1/4)*(A[1, 2*j-1]^2-1)^2/(b*A[1, 2*j]), n); A[1, 2*j+2] := `mod`((1/2)*(3*A[1, 2*j-1]+a)*(3*A[1, 2*j-1]^2+2*a*A[1, 2*j-1]+1)/(b*A[1, 2*j])+(-1/8)*b*(3*A[1, 2*j-1]^2+2*a*A[1, 2*j-1]+1)^3/(b^3*A[1, 2*j]^3)-A[1, 2*j], n) else return gcd(2*b*A[1, 2*j], n) end if end do; m := 1; for i to r do if Z[i] = 0 then m := m+1 end if end do; x := A[1, 2*m-1]; y := A[1, 2*m]; Z[m] := 0; for l to r do if Z[l] = 1 then X := A[1, 2*l-1]; Y := A[1, 2*l]; if gcd(X-x, n) = 1 then lambda := `mod`((Y-y)/(X-x), n); x3 := b*lambda^2-a-x-X; y3 := (2*x+X+a)*(Y-y)/(X-x)-b*lambda^3-y; x := `mod`(x3, n); y := `mod`(y3, n) elif gcd(X-x, n) < n then return gcd(X-x, n) elif gcd(X-x, n) = n then (D(print))("refaire") end if end if end do; [x, y] end proc; 1}{\[\]}
\end{mapleinput}
\mapleresult
\begin{maplelatex}
\mapleinline{inert}{2d}{double2 := proc (k, x1, y1, a, b, n) local r, X, Z, A, m, lambda, nu, i, x, y, Y, x3, y3, j, l; X := convert(k, 'base', 2); Z := convert(X, array); r := 0; while 2^r <= k do r := r+1 end do; A := Matrix(1, 2*r); A[1, 1] := x1; A[1, 2] := y1; for j to r-1 do if gcd(2*b*A[1, 2*j], n) = 1 then A[1, 2*j+1] := `mod`((1/4)*(A[1, 2*j-1]^2-1)^2/(b*A[1, 2*j]), n); A[1, 2*j+2] := `mod`(((3/2)*A[1, 2*j-1]+(1/2)*a)*(3*A[1, 2*j-1]^2+2*a*A[1, 2*j-1]+1)/(b*A[1, 2*j])+(-1/8)*b*(3*A[1, 2*j-1]^2+2*a*A[1, 2*j-1]+1)^3/(b^3*A[1, 2*j]^3)-A[1, 2*j], n) else return gcd(2*b*A[1, 2*j], n) end if end do; m := 1; for i to r do if Z[i] = 0 then m := m+1 end if end do; x := A[1, 2*m-1]; y := A[1, 2*m]; Z[m] := 0; for l to r do if Z[l] = 1 then X := A[1, 2*l-1]; Y := A[1, 2*l]; if gcd(X-x, n) = 1 then lambda := `mod`((Y-y)/(X-x), n); x3 := b*lambda^2-a-x-X; y3 := (2*x+X+a)*(Y-y)/(X-x)-b*lambda^3-y; x := `mod`(x3, n); y := `mod`(y3, n) elif gcd(X-x, n) < n then return gcd(X-x, n) elif gcd(X-x, n) = n then (D(print))("refaire") end if end if end do; [x, y] end proc}{\[\displaystyle {\it double2}\, := \,\textbf{proc} (kx1y1abn) \\
\textbf{local} \,r,\,X,\,Z,\,A,\,m,\,lambda,\,nu,\,i,\,x,\,y,\,Y,\,x3,\,y3,\,j,\,l; \\
\mapleIndent{1} X\,:=\,\mathit{convert} (k,\,'base',\,2);\\
\mapleIndent{1} Z\,:=\,\mathit{convert} (X,\,array);\\
\mapleIndent{1} r\,:=\,0;\\
...
\mapleIndent{1} A[1,\,2]\,:=\,y1;\\
\mapleIndent{1} \textbf{for} \,j \,\textbf{to} \,r + -1 \,\textbf{do}\\
\mapleIndent{2} \textbf{if} \,\mathit{gcd} (2 \ast b \ast A[1,\,2 \ast j],\,n) \, = \, 1 \, \textbf{then} \\
\mapleIndent{3} A[1,\,2 \ast j + 1]\,:=\,\mathit{mod} (1/4 \ast A[1,\,2 \ast j + -1]\hat{~}{2} + -1\hat{~}{2} \ast b\hat{~}{-1} \ast A[1,\,2 \ast j]\hat{~}{-1},\,n);\\
\mapleIndent{3} A[1,\,2 \ast j + 2]\,:=\,\mathit{mod} ((3/2 \ast A[1,\,2 \ast j + -1] + 1/2 \ast a) \ast (3 \ast A[1,\,2 \ast j + -1]\hat{~}{2} + 2 \ast a \ast A[1,\,2 \ast j + -1] + 1) \ast b\hat{~}{-1} \ast A[1,\,2 \ast j]\hat{~}{-1}-1/8 \ast b \ast 3 \ast A[1,\,2 \ast j + -1]\hat{~}{2} + 2 \ast a \ast A[1,\,2 \ast j + -1] + 1\hat{~}{3} \ast b\hat{~}{-3} \ast A[1,\,2 \ast j]\hat{~}{-3}-A[1,\,2 \ast j],\,n)\\
\mapleIndent{2} \textbf{else}\\
\mapleIndent{3} \textbf{return}\,\mathit{gcd} (2 \ast b \ast A[1,\,2 \ast j],\,n)\\
\mapleIndent{2} \textbf{end\ if}; \\
\mapleIndent{1} \textbf{end\ do};;\\
\mapleIndent{1} m\,:=\,1;\\
\mapleIndent{1} \textbf{for} \,i \,\textbf{to} \,r \,\textbf{do}\\
...
\mapleIndent{3} \textbf{if} \,\mathit{gcd} (X-x,\,n) \, = \, 1 \, \textbf{then} \\
\mapleIndent{4} lambda\,:=\,\mathit{mod} ((Y-y) \ast X-x\hat{~}{-1},\,n);\\
\mapleIndent{4} x3\,:=\,b \ast lambda\hat{~}{2}-a-x-X;\\
\mapleIndent{4} y3\,:=\,(2 \ast x + X + a) \ast (Y-y) \ast X-x\hat{~}{-1}-b \ast lambda\hat{~}{3}-y;\\
\mapleIndent{4} x\,:=\,\mathit{mod} (x3,\,n);\\
\mapleIndent{4} y\,:=\,\mathit{mod} (y3,\,n)\\
\mapleIndent{3} \textbf{elif} \,\mathit{gcd} (X-x,\,n)<n \, \textbf{then} \\
\mapleIndent{4} \textbf{return}\,\mathit{gcd} (X-x,\,n)\\
...
\begin{maplegroup}
\begin{mapleinput}
\mapleinline{active}{2d}{Lenstra2 := proc (n) local b, Q, p, a, x1, y1; if n::prime then return print("n est premier") end if; a := rand(); x1 := rand(); y1 := rand(); if gcd(y1, n) = 1 then b := `mod`((a*x1^2+x1^3+x1)/y1^2, n) elif 1 < gcd(y1, n) then return gcd(y1, n) end if; for p from 2 to factorial(1000) do Q := double2(factorial(p), x1, y1, a, b, n); if Q::integer then return Q end if; a := rand(); x1 := rand(); y1 := rand(); b := `mod`((a*x1^2+x1^3+x1)/y1^2, n); if 1 < gcd(y1, n) then return gcd(y1, n) end if end do; print("RÃ©essayer avec une nouvelle courbe et/ou point"); Q end proc; 1}{\[\]}
\end{mapleinput}
\mapleresult
\begin{maplelatex}
\mapleinline{inert}{2d}{Lenstra2 := proc (n) local b, Q, p, a, x1, y1; if n::prime then return print("n est premier") end if; a := rand(); x1 := rand(); y1 := rand(); if gcd(y1, n) = 1 then b := `mod`((a*x1^2+x1^3+x1)/y1^2, n) elif 1 < gcd(y1, n) then return gcd(y1, n) end if; for p from 2 to factorial(1000) do Q := double2(factorial(p), x1, y1, a, b, n); if Q::integer then return Q end if; a := rand(); x1 := rand(); y1 := rand(); b := `mod`((a*x1^2+x1^3+x1)/y1^2, n); if 1 < gcd(y1, n) then return gcd(y1, n) end if end do; print("RÃ©essayer avec une nouvelle courbe et/ou point"); Q end proc}{\[\displaystyle {\it Lenstra2}\, := \,\textbf{proc} (n) \\
...
\mapleIndent{1} x1\,:=\,\mathit{rand} ();\\
\mapleIndent{1} y1\,:=\,\mathit{rand} ();\\
\mapleIndent{1} \textbf{if} \,\mathit{gcd} (y1,\,n) \, = \, 1 \, \textbf{then} \\
\mapleIndent{2} b\,:=\,\mathit{mod} ((a \ast x1\hat{~}{2} + x1\hat{~}{3} + x1) \ast y1\hat{~}{-2},\,n)\\
\mapleIndent{1} \textbf{elif} \,1<\mathit{gcd} (y1,\,n) \, \textbf{then} \\
\mapleIndent{2} \textbf{return}\,\mathit{gcd} (y1,\,n)\\
\mapleIndent{1} \textbf{end\ if}; ;\\
\mapleIndent{1} \textbf{for} \,p \,\textbf{from} \,2 \,\textbf{to} \,\mathit{factorial} (1000) \,\textbf{do}\\
\mapleIndent{2} Q\,:=\,\mathit{double2} (\mathit{factorial} (p),\,x1,\,y1,\,a,\,b,\,n);\\
\mapleIndent{2} \textbf{if} \,Q::integer \, \textbf{then} \\
\mapleIndent{3} \textbf{return}\,Q\\
\mapleIndent{2} \textbf{end\ if}; ;\\
...
\mapleIndent{2} y1\,:=\,\mathit{rand} ();\\
\mapleIndent{2} b\,:=\,\mathit{mod} ((a \ast x1\hat{~}{2} + x1\hat{~}{3} + x1) \ast y1\hat{~}{-2},\,n);\\
\mapleIndent{2} \textbf{if} \,1<\mathit{gcd} (y1,\,n) \, \textbf{then} \\
\mapleIndent{3} \textbf{return}\,\mathit{gcd} (y1,\,n)\\
\mapleIndent{2} \textbf{end\ if}; \\
\mapleIndent{1} \textbf{end\ do};;\\
\mapleIndent{1} \mathit{print} (Réessayer\ avec\ une\ nouvelle\ courbe\ et/ou\ point'');\\
\mapleIndent{1} Q\\
...
\begin{maplegroup}
\begin{mapleinput}
\mapleinline{active}{2d}{card(0, 3-4, 166, 3)}{\[{\it card} \left( 0,-1,166,3 \right) \]}
\end{mapleinput}
\mapleresult
\begin{maplelatex}
